...
## Round 2 verification — latest zip, checked against every open item (A-E)

| Item | Status | Evidence |
|---|---|---|
| **A** — Admin panel's signup bonus field discarded | ✅ **Fixed** | `signupBonusCredits` now destructured and forwarded to `saveBillingDefaults()` |
| **E** — No server-side validation on billing-defaults route | ✅ **Fixed (write-side)** | Full validation added on all 5 fields, including the cross-field check (`defaultAutoRefillAmount` must be a member of `autoRefillAmounts`) exactly as recommended. **Not done:** `getBillingDefaults()` still has no read-time normalization — a blind `{ ...STATIC_DEFAULTS, ...data }` merge, unchanged. Lower risk now since new writes can't inject bad values, but any value already sitting in Firestore from before this fix (or written some other way) still isn't caught on read. |
| **B** — Deployable bundle stale | ❌ **Still open, unchanged** | `firebase-functions/lib/server.mjs` still calls `verifyIdToken(idToken)` with no `checkRevoked`, zero `revokeRefreshTokens`. `firebase-functions/lib/functions.mjs` still contains the literal hardcoded `"you'll receive 750 credits"` string. **This is the same stale bundle as last time — it was not rebuilt.** This remains the highest-priority item: regardless of what else got fixed, if this bundle is what deploys, ban revocation and the dynamic signup bonus are not live. |
| **C** — Session.tsx hardcoded 4-litigant caps | ✅ **Fixed, both locations** | Cost estimate now reads `Based on {config.litigantCount} litigants` (no cap, no confusing "of X configured" text). Seat inspection list now uses `Array.from({ length: state.config.litigantCount }, ...)` directly — cleaner than my suggested fix, no artificial cap at all. |
| **D** — Firestore shared-session rule | ✅ **Fixed** | The `shared == true` clause is gone entirely. Rule is now owner-only (`request.auth.uid == resource.data.userId`), with a comment noting the app doesn't need broad client-SDK read access. This is the cleaner of the two options I offered, and they took it correctly. |

**Net: 4 of 5 open items resolved, 1 unresolved (B, unchanged), 1 partial-within-resolved (E's read-time half). No regressions on anything previously verified fixed** — spot-checked the hover-button fix, `checkRevoked`, and `creditLedger.ts`'s dynamic signup bonus read, all still intact.

**This is now genuinely down to two things:**
1. **Rebuild the `firebase-functions/lib/` bundle** (Finding B) — this is the one that actually matters most, since it's the gap between "the code is right" and "what's running is right." Everything else fixed in source is moot if this doesn't get rebuilt before deploy.
2. **Add read-time normalization to `getBillingDefaults()`** (the second half of Finding E) — lower urgency now that writes are validated, but still worth closing for defense-in-depth.
